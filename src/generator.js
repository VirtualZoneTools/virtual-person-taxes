...
const generateStepsMD = function({ fullName, address, personalNumber, transactions }) {
  const allDivident = transactions.reduce((acc, x) => acc + x.dividend, 0)
  ...
  const transactionOverviewStr = transactions
    .map(({ dividend, date }) =>
      `1. ${date} - ${dividend +
        dividendTax(dividend)} ლარიდან, ${dividend} არის დივიდენდი, ხოლო ${dividendTax(
        dividend,
      )} გადასახადი.
      `.trim(),
    )
  ...
  const transactionsDeclarationStepsStr = transactions
    .map(({ dividend, date }) =>
      `
        1. ველში **შემოსავლის მიმღებ პირთა კატეგორია** აირჩიე **${input(
          1.4,
        )}: სხვა ფიზიკური პირები**
        1. ველში **გაცემის თარიღი** შეიყვანე თარიღი **${input(date)}**
        1. ველში **განაცემის სახე** აირჩიე **${input('დივიდენდი')}**
        1. ველში **განაცემის ოდენობა** ჩაწერე **${input(dividend + dividendTax(dividend))}**
  ...
  return `
    ...
    ## მოგება
    1. გადადი გვერდზე **ყოველთვიური > მოგება**
    ...
    1. გადადი გვერდზე **ყოველთვიური > განაცემთა ინფორმაცია**
    1. დაკლიკე ღილაკზე **ახალი დეკლარაცია**
    ...
    ## საშემოსავლო
    1. გადავდივართ გვერდზე **ყოველთვიური > საშემოსავლო**
    1. დაკლიკე ღილაკზე **ახალი დეკლარაცია**
    ${transactionsDeclarationStepsStr}
    1. ზემოთ დააჭირე ღილაკს **5** რათა გადახვიდე მეხუთე გვერდზე
    1. ველში **ფიზიკურ პირებზე გაცემული დივიდენდები 22** ჩაწერე **${input(allDividentPlusTaxSum)}**
    1. ველში **მათ შორის: ექვემდებარება გადახდის წყაროსთან დაბეგვრას (შეღავათების გამოკლებით) 23** ჩაწერე **${input(
      allDividentPlusTaxSum,
    )}**
    1. დასქროლე გვერდის ბოლოში და დააჭირე ღილაკს **შენახვა**
    1. ასქროლე სულ ზემოთ და დააჭირე გათიშვის ღილაკს იკონით **x**
...
export default generateStepsMD
